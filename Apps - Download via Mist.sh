#!/bin/zsh
#
# Purpose: Download OS updates via mist
#
...
logMe "Successful download..."
logMe "Clearing the Quarantine flag"
/usr/bin/xattr -d -r com.apple.quarantine "${destPath}/${InstallFile}" 2> /dev/null
# Perform the new Gatekeeper scan if on Sonoma or higher
logMe "Performing Gatekeeper Scan"
/usr/bin/gktool scan "${destPath}/${InstallFile}"
