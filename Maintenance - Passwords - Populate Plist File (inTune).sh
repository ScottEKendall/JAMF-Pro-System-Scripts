...
# 
# 1.0 - Initial code
# 1.1 - Add logic to write out PasswordAge as well to the plist file
#
######################################################################################################
#
...
passwordAge=$(echo $recordExtensions | jq -r '.extensionAttributes[] | select(.name == "Password Age") | .values[]' )
logMe "INFO: inTune password date shows: $newPasswordDate"
logMe "INFO: Curent Password Age: $passwordAge"
